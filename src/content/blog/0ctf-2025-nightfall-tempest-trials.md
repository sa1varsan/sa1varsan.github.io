---
title: '0ctf 2025 - Nightfall Tempest Trials'
description: '利用带噪 NTT 阶段泄露逐层反推 Kyber secret，多项式拼接后恢复 AES 密钥。'
timeLabel: 2025
timeOrder: 2025
...
- 对 Kyber1024 的 keygen 进行了补丁：使用了非标准的向量长度 $K = 12$（因此 `PK_BYTES = 12 × 384 + 32 = 4640`）。
- `ref/ntt.c` 会打印每一层 NTT（Number Theoretic Transform）阶段的所有数据，每个多项式泄露 $7 \times 256 = 1792$ 个值（见 `attachment/ntt.patch`）。
- `task.py` 仅保留前 $K$ 个泄露块（对应 secret polys），并在每个 stage 加入有界噪声 $\Delta = [240, 430, 600, 75, 70, 88, 99]$；随后对打包后的 NTT 域 secret 做哈希以派生 AES-CBC 密钥，并输出 `pk`、带噪声的 `leaks`、`ct`、`iv`。
...
$t = \zeta \cdot v,\qquad a = u + t,\qquad b = u - t,$

其中运算模数为 q = 3329。补丁后的代码会在每个 stage 泄露 a', b'，并满足

$|a' - a|,\ |b' - b| \le \Delta_s$

其中 stage 对应的 $\Delta_s$ 就是上面的那组 $\Delta$。第一阶段输入的系数很小（$\eta_1 = 2 \Rightarrow u,v \in {-2,-1,0,1,2}$），这为逆向 NTT 提供了很强的约束。

## 分阶段重构（Stage-Wise Reconstruction）

- **允许的剩余类集合（Allowed residue sets）：**
    
    对于每个 stage $s$ 的观测值 $o_{s,i}$，构造
    
    $S_{s,i} = {(o_{s,i} + d) \bmod q \mid d \in [-\Delta_s,\Delta_s]}.$
    
- **反向 butterfly（Reverse butterflies）：**
    
    从最后一个 stage（`len = 2`）开始，按长度 $4, 8, 16, 32, 64$ 逐层回退。用
    
    $u = \tfrac{A+B}{2},\qquad v = \tfrac{(A-B),\zeta^{-1}}{2},$
    
    计算前一层可能的值，并只保留与 $S_{s-1}$ 相容的解（在 `crypto/Nightfall Tempest Trials/solve/solve.py` 的 `reverse_stage` 中实现）。
    
- **第一阶段的小系数约束（First stage constraint）：**
    
    在 `len = 128` 时，额外与小系数集合 $\{0, \pm 1, \pm 2\}$ 求交（`reverse_final_small`）。
...
2. 计算 $k = \mathrm{SHA256}(\texttt{sk\_bytes})$，并使用给定 IV 尝试 AES-CBC 解密。
